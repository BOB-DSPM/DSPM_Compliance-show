...
REQ_SPEC = HeaderSpec(
    ...
    aliases={
        ...
        "해당컴플": ["해당컴플", "해당 컴플", "applicable", "applicable_framework", "applicable_flag"],
    },
)
...
def normalize_header_map(fieldnames: List[str], spec: HeaderSpec) -> Dict[str, str]:
    ...
    if missing:
        ...
        # 필수는 최소한 하나라도 실제 필드명과 동일한지 마지막 체크
        for r in missing[:]:
            if r in fieldnames:
                mapping[r] = r
                missing.remove(r)
...
def load_requirements(db: Session, req_csv: Path, dialect: Dict[str, Any], encoding: str, merge_mode: str):
    with req_csv.open("r", encoding=encoding, newline="") as f:
        ...
        for row in reader:
            ...
            applicable = getv(row, hdrmap, "해당컴플") or None  # 선택 사용(모델 필드가 없으면 보관만)

            r = upsert_requirement(
                db, framework_code, item_code, title, description,
                mapping_status, auditable, audit_method, merge_mode=merge_mode
            )
            # 필요 시 r에 applicable 저장하려면 모델 필드 추가 후: r.applicable = applicable
